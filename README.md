...
It is an SVG-first tool with front elevation, isometric, and top/plan inspection views. The application separates configurable locker definitions, deterministic layout optimization, scoring, manufacturing derivation, and the dashboard UI so a product engineer can inspect the trade-offs behind each alternative.
...
1. **Project setup** — set the project name, demand mix, exact columns, exact rows, and ground clearance from the floor. The grid must contain exactly the requested number of boxes.
...
4. **Generate layout** — calculate the complete cabinet envelope from the filled grid, then inspect it from the front, isometric, or top/plan view.
...
The optimizer requires `rows × columns` to equal total demand, fills every cell, calculates each column width from the widest box in that column, calculates each row height from the tallest box in that row, and derives the complete fabrication envelope. Ground clearance is installation metadata: it changes the installed top elevation, not the fabricated locker geometry. One existing top-row cell is marked controller-ready for later assignment; it never adds cabinet width.
...
The test suite covers default demand, invalid constraints, mixed-size packing, exact locker counts, deterministic ranking, score bounds, manufacturing quantities, JSON export, CSV export, reducer edits, ground-clearance calculations, and generation state.
...
- `src/domain/dimensions.js` — fabrication envelope and installed-height calculations
...
Backend persistence, authentication, electrical schematics, finite-element validation, live CAD export, and real hardware integrations are not part of this local prototype. The current isometric view is an inspection-oriented SVG projection rather than a Three.js/CAD model.
