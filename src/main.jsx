...
function ConstraintsView({ state, actions, onGenerate }) {
  ...
  return <div className="view"><SectionHeader eyebrow="03 / engineering envelope" title="Set the hard boundaries." description="The optimizer rejects configurations that exceed these production and site constraints." action={<button className="button primary" onClick={onGenerate}><Icon name="spark" size={16} />Recalculate</button>} /><StatusBanner state={state} /><div className="constraint-layout"><section className="panel"><div className="panel-heading"><div><span className="eyebrow">Cabinet envelope</span><h2>Maximum outer dimensions</h2></div><span className="panel-code">LIMITS / MM</span></div><div className="field-grid three"><Field label="Maximum width" value={c.maxWidthMm} onChange={(value) => actions.updateConstraints('maxWidthMm', value)} hint="Covers the complete locker frame." /><Field label="Maximum height" value={c.maxHeightMm} onChange={(value) => actions.updateConstraints('maxHeightMm', value)} hint="Floor to cabinet top." /><Field label="Maximum depth" value={c.maxDepthMm} onChange={(value) => actions.updateConstraints('maxDepthMm', value)} hint="Locker depth included." /></div><div className="panel-heading sub"><div><span className="eyebrow">Construction rules</span><h2>Module allowances</h2></div></div><div className="field-grid four"><Field label="Divider" value={c.dividerMm} onChange={(value) => actions.updateConstraints('dividerMm', value)} /><Field label="Frame" value={c.frameMm} onChange={(value) => actions.updateConstraints('frameMm', value)} /><Field label="Door gap" value={c.doorGapMm} onChange={(value) => actions.updateConstraints('doorGapMm', value)} /><Field label="Controller package" value={c.controllerWidthMm} onChange={(value) => actions.updateConstraints('controllerWidthMm', value)} hint="Must fit inside the later-assigned top-row locker." /></div><Field label="Preferred module width" value={c.preferredModuleWidthMm} onChange={(value) => actions.updateConstraints('preferredModuleWidthMm', value)} hint="Used to bound the search space; not a hard limit." /></section><aside className="panel assumption-panel"><span className="eyebrow">Design assumptions</span><h2>Production defaults</h2><div className="assumption"><span>01</span><div><strong>Sheet metal first</strong><p>Laser-cut and press-brake formed panels with powder coat after deburring.</p></div></div><div className="assumption"><span>02</span><div><strong>Controller stays in-frame</strong><p>One existing top-row locker is assigned later; it never adds a bay, column, or width.</p></div></div><div className="assumption"><span>03</span><div><strong>Modules stay repeatable</strong><p>Repeated widths and vertical service channels reduce assembly variance.</p></div></div></aside></div></div>;
}
