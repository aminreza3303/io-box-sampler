...
function LockerLayoutSvg({ candidate, lockerTypes, constraints, view = 'front', compact = false }) {
  ...
  const gap = constraints.dividerMm;
  const widths = candidate.columnWidths || Array.from({ length: candidate.columns }, () => Math.max(...lockerTypes.map((type) => type.dimensions.width)));
  const heights = candidate.rowHeights || Array.from({ length: candidate.rows }, () => Math.max(...lockerTypes.map((type) => type.dimensions.height)));
  const W = candidate.dimensions.width;
  const H = candidate.dimensions.height;
  const D = candidate.dimensions.depth;
  const G = candidate.finalDimensions?.groundClearanceMm || 0;
  const topY = frame + gap;
  const dimensionX = W + 36;
  const dimensionY = H + 34;
  const xAt = (column) => frame + gap + widths.slice(0, column).reduce((sum, value) => sum + value + gap, 0);
  const yAt = (row) => topY + heights.slice(0, row).reduce((sum, value) => sum + value + gap, 0);
  const lockerAt = (row, column) => candidate.lockers.find((locker) => locker.row === row && locker.column === column);
  const cell = (locker, x, y, width, height, compactCell = false) => {
    const type = types.get(locker.typeId);
    const controllerReady = locker.id === candidate.controllerSlotId;
    return <g className={'svg-locker ' + (controllerReady ? 'controller-ready' : '')} key={locker.id} tabIndex="0"><title>{type.name + ' locker ' + locker.id + (controllerReady ? ', top-row controller-ready slot' : '') + ', ' + type.dimensions.width + ' × ' + type.dimensions.height + ' × ' + type.dimensions.depth + ' mm'}</title><rect x={x} y={y} width={width} height={height} rx="2" fill={colors[locker.typeId] || '#7e90d5'} /><rect className="svg-door" x={x + 5} y={y + 5} width={width - 10} height={Math.max(height - 10, 5)} rx="2" />{controllerReady && <rect className="svg-controller-ready" x={x + 3} y={y + 3} width={width - 6} height={Math.max(height - 6, 7)} rx="2" />}{controllerReady && !compactCell && <text className="svg-ready-label" x={x + width / 2} y={y + 19} textAnchor="middle">CTRL READY</text>}<circle className="svg-lock" cx={x + width - 15} cy={y + Math.min(height / 2, 16)} r="3" /><text className="svg-code" x={x + 16} y={y + Math.min(height / 2 + 5, height - 6)}>{type.code}</text>{!compactCell && <text className="svg-size" x={x + width - 23} y={y + Math.min(height / 2 + 4, height - 6)} textAnchor="end">{type.dimensions.width + '×' + type.dimensions.height}</text>}</g>;
  };
  const renderFrontCells = (offsetX, offsetY, compactCells = compact) => Array.from({ length: candidate.rows }, (_, row) => Array.from({ length: candidate.columns }, (_, column) => {
    const locker = lockerAt(row, column);
    return cell(locker, offsetX + xAt(column), offsetY + yAt(row), widths[column], heights[row], compactCells);
  }));
  if (view === 'top') {
    const planX = 60;
    const planY = 42;
    return <svg className={'layout-svg plan-svg ' + (compact ? 'compact' : '')} viewBox={'0 0 ' + (W + 120) + ' ' + (D + 100)} role="img" aria-label={'Top view, ' + W + ' by ' + D + ' millimetres'}>
      <defs><pattern id="topGridPattern" width="24" height="24" patternUnits="userSpaceOnUse"><path d="M24 0H0V24" fill="none" stroke="#dbe7e6" strokeWidth="0.7" /></pattern></defs>
      <rect x="0" y="0" width={W + 120} height={D + 100} fill="url(#topGridPattern)" opacity=".5" /><rect className="svg-top-frame" x={planX} y={planY} width={W} height={D} rx="5" />
      {widths.map((width, column) => { const x = planX + frame + gap + widths.slice(0, column).reduce((sum, value) => sum + value + gap, 0); const locker = lockerAt(0, column); const ready = locker.id === candidate.controllerSlotId; return <g key={'plan-column-' + column}><rect className="svg-top-cell" x={x} y={planY + frame} width={width} height={D - frame * 2} fill={colors[locker.typeId] || '#7e90d5'} opacity=".72" />{ready && <rect className="svg-controller-ready" x={x + 3} y={planY + frame + 3} width={width - 6} height={D - frame * 2 - 6} rx="2" />}<text className="svg-plan-code" x={x + width / 2} y={planY + D / 2} textAnchor="middle">{ready && !compact ? 'CTRL' : 'C' + (column + 1)}</text></g>; })}<line className="svg-dimension" x1={planX} y1={planY + D + 25} x2={planX + W} y2={planY + D + 25} /><text className="svg-dimension-label" x={planX + W / 2} y={planY + D + 40} textAnchor="middle">{W} mm width</text><line className="svg-dimension" x1={planX + W + 25} y1={planY} x2={planX + W + 25} y2={planY + D} /><text className="svg-dimension-label" transform={'translate(' + (planX + W + 40) + ' ' + (planY + D / 2) + ') rotate(90)'} textAnchor="middle">{D} mm depth</text><text className="svg-axis-label" x={planX + W / 2} y={planY - 14} textAnchor="middle">TOP / PLAN VIEW · {candidate.columns} COLUMNS</text>
    </svg>;
  }
  if (view === 'isometric') {
    const dx = D * 0.55;
    const dy = D * 0.32;
    const isoX = 70;
    const groundY = H + G + 86;
    const isoY = groundY - G - H;
    const topBackY = isoY - dy;
    const installedTop = candidate.finalDimensions?.installedTopMm || H + G;
    return <svg className={'layout-svg iso-svg ' + (compact ? 'compact' : '')} viewBox={'0 0 ' + (W + dx + 170) + ' ' + (groundY + 70)} role="img" aria-label={'Isometric view, ' + W + ' by ' + H + ' by ' + D + ' millimetres'}>
      <defs><pattern id="isoGridPattern" width="24" height="24" patternUnits="userSpaceOnUse"><path d="M24 0H0V24" fill="none" stroke="#dbe7e6" strokeWidth="0.7" /></pattern></defs>
      <rect x="0" y="0" width={W + dx + 170} height={groundY + 70} fill="url(#isoGridPattern)" opacity=".45" />
      <polygon className="iso-top-face" points={isoX + ',' + isoY + ' ' + (isoX + W) + ',' + isoY + ' ' + (isoX + W + dx) + ',' + topBackY + ' ' + (isoX + dx) + ',' + topBackY} />
      <polygon className="iso-side-face" points={(isoX + W) + ',' + isoY + ' ' + (isoX + W + dx) + ',' + topBackY + ' ' + (isoX + W + dx) + ',' + (groundY - dy) + ' ' + (isoX + W) + ',' + (groundY - G)} />
      <rect className="iso-front-frame" x={isoX} y={isoY} width={W} height={H} rx="4" />
      <g>{renderFrontCells(isoX, isoY, compact)}</g>
      <line className="iso-ground" x1="30" y1={groundY} x2={W + dx + 130} y2={groundY} /><line className="iso-leg" x1={isoX + 28} y1={groundY - G} x2={isoX + 28} y2={groundY} /><line className="iso-leg" x1={isoX + W - 28} y1={groundY - G} x2={isoX + W - 28} y2={groundY} />
      {!compact && <><text className="iso-label" x={isoX + W / 2} y={groundY + 25} textAnchor="middle">GROUND / 0</text><text className="iso-label" x={isoX + W / 2} y={isoY - dy - 15} textAnchor="middle">TOP ELEVATION {installedTop} mm</text><text className="svg-dimension-label" x={isoX + W / 2} y={groundY + 42} textAnchor="middle">{W} mm width</text><text className="svg-dimension-label" transform={'translate(' + (isoX + W + dx + 28) + ' ' + (groundY - G / 2) + ') rotate(90)'} textAnchor="middle">{G} mm ground clearance</text><text className="svg-dimension-label" transform={'translate(' + (isoX + W + dx + 45) + ' ' + (isoY + (H / 2)) + ') rotate(90)'} textAnchor="middle">{H} mm cabinet height</text><text className="svg-dimension-label" x={isoX + W + dx / 2} y={topBackY - 10} textAnchor="middle">{D} mm depth</text></>}
    </svg>;
  }
  return <svg className={'layout-svg front-svg ' + (compact ? 'compact' : '')} viewBox={'0 0 ' + (W + 80) + ' ' + (H + 70)} role="img" aria-label={'Front view, ' + candidate.columns + ' columns by ' + candidate.rows + ' rows'}>
    <defs><pattern id="frontGridPattern" width="24" height="24" patternUnits="userSpaceOnUse"><path d="M24 0H0V24" fill="none" stroke="#dbe7e6" strokeWidth="0.7" /></pattern></defs>
    <rect x="0" y="0" width={W + 80} height={H + 70} fill="url(#frontGridPattern)" opacity=".5" /><rect className="svg-frame" x={frame} y={frame} width={W - frame * 2} height={H - frame * 2} rx="5" /><g>{renderFrontCells(0, 0, compact)}</g>
    {!compact && <><line className="svg-dimension" x1={frame} y1={dimensionY} x2={W - frame} y2={dimensionY} /><text className="svg-dimension-label" x={W / 2} y={dimensionY + 14} textAnchor="middle">{W} mm overall width</text><line className="svg-dimension" x1={dimensionX} y1={frame} x2={dimensionX} y2={H - frame} /><text className="svg-dimension-label" transform={'translate(' + (dimensionX + 15) + ' ' + (H / 2) + ') rotate(90)'} textAnchor="middle">{H} mm cabinet height</text><text className="svg-axis-label" x={W / 2} y={H - 9} textAnchor="middle">FRONT / EVERY CELL FILLED</text></>}
  </svg>;
}
...
function ViewToggle({ value, onChange }) {
  return <div className="view-toggle" role="tablist" aria-label="Cabinet inspection view"><button className={value === 'front' ? 'active' : ''} onClick={() => onChange('front')} role="tab" aria-selected={value === 'front'}>Front</button><button className={value === 'isometric' ? 'active' : ''} onClick={() => onChange('isometric')} role="tab" aria-selected={value === 'isometric'}>Isometric</button><button className={value === 'top' ? 'active' : ''} onClick={() => onChange('top')} role="tab" aria-selected={value === 'top'}>Top / plan</button></div>;
}

function FinalDimensionsCard({ finalDimensions }) {
  if (!finalDimensions) return null;
  return <section className="final-dimensions"><div><span>Fabrication envelope</span><strong>{number(finalDimensions.cabinetWidthMm)} × {number(finalDimensions.cabinetHeightMm)} × {number(finalDimensions.cabinetDepthMm)}<small>mm</small></strong></div><div><span>Ground clearance</span><strong>{number(finalDimensions.groundClearanceMm)}<small>mm</small></strong></div><div><span>Installed top elevation</span><strong>{number(finalDimensions.installedTopMm)}<small>mm</small></strong></div><div><span>Footprint</span><strong>{number(finalDimensions.footprintAreaM2, 2)}<small>m²</small></strong></div></section>;
}
