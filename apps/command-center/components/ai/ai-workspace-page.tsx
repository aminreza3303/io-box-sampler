...
type AgentRun = { id: string; runtime: string; prompt: string; status: "QUEUED" | "RUNNING" | "SUCCEEDED" | "BLOCKED" | "FAILED"; resultKind?: string | null; output?: string | null; error?: string | null; context?: { processTitle?: string } | null; agentSession?: { sessionName: string; project: { name: string; code: string } } | null; createdAt: string; completedAt?: string | null; actor?: { displayName: string }; messages: AgentMessage[] };
type Project = { id: string; name: string; code: string };
type SessionStatus = { project: Project; session: { id: string; sessionName: string; status: string; lastRunAt?: string | null } | null; workspace: { key: string; available: boolean; error?: string } };
type ChatPayload = { runId?: string; kind?: string; output?: string; error?: string; proposal?: { id: string; title: string; status: string } | null; session?: { id: string; sessionName: string; status: string; workspaceKey: string; project: Project } | null };
...
function friendlyAgentError(error: string) {
  if (/executable|spawn .*ENOENT|not found/i.test(error)) return "CLI هرمس پیدا نشد. Hermes را نصب کنید یا مسیر آن را در HERMES_EXECUTABLE داخل .env.local قرار دهید و سرویس را restart کنید.";
  return error;
}

function HermesSessionPanel({ projects, selectedProjectId, onProjectChange, status, proposal }: { projects: Project[]; selectedProjectId: string; onProjectChange: (value: string) => void; status: SessionStatus | null; proposal: ChatPayload["proposal"] }) {
  return <section className="rounded-2xl border border-violet-100 bg-violet-50 p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h2 className="font-black text-violet-950">اتصال پروژه</h2><p className="mt-1 text-xs leading-6 text-violet-900">Hermes در workspace پروژه و session پایدار آن کار می‌کند.</p></div><Badge tone={status?.workspace.available === false ? "warning" : "info"}>{status?.workspace.available === false ? "workspace مسدود" : "Hermes مادر"}</Badge></div><label className="mt-4 block text-xs font-bold text-violet-900">پروژهٔ کاری<select value={selectedProjectId} onChange={(event) => onProjectChange(event.target.value)} className="mt-2 w-full rounded-xl border border-violet-200 bg-white px-3 py-2 text-sm font-bold text-slate-900 outline-none focus:ring-2 focus:ring-violet-300"><option value="">بدون پروژه / one-shot</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label>{selectedProjectId && <p className={`mt-3 rounded-xl p-3 text-xs leading-6 ${status?.workspace.available ? "bg-emerald-100 text-emerald-900" : "bg-amber-100 text-amber-900"}`}>{status?.workspace.available ? <>{status.session ? <>session فعال: {status.session.sessionName}</> : <>با ارسال پیام، session این پروژه ساخته می‌شود.</>} · workspace: {status.workspace.key}</> : status?.workspace.error ?? "در حال بررسی workspace…"}</p>}{proposal && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs leading-6 text-emerald-900"><strong className="block">proposal آمادهٔ بررسی</strong><span>{proposal.title} · هنوز task ساخته نشده است.</span><Link href="/ceo/proposals" className="mt-2 inline-block font-black underline">رفتن به اتاق مدیرعامل ←</Link></div>}</section>;
}
...
export function AiWorkspacePage() {
  ...
  const [proposal, setProposal] = useState<ChatPayload["proposal"]>(null);
  ...
  useEffect(() => { void Promise.all([loadRuns(), fetch("/api/snapshot").then((response) => response.ok ? response.json() : Promise.reject(new Error("دریافت پروژه‌ها ناموفق بود."))).then((data: { projects?: Project[] }) => { setProjects(data.projects ?? []); setSelectedProjectId((current) => current || data.projects?.find((project) => project.code === "newcash")?.id || data.projects?.[0]?.id || ""); })]).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "دریافت اجراها ناموفق بود.")); }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const response = await fetch("/api/ai/chat", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message: trimmed, context, processId, projectId: selectedProjectId || undefined }) });
      ...
      await loadRuns();
      if (payload.kind === "blocked") setError(friendlyAgentError(payload.error ?? "Hermes برای این محیط فعال نیست."));
      if (payload.kind === "error") setError(friendlyAgentError(payload.error ?? "اجرای دستیار ناموفق بود."));
    } catch (reason) {
      setError(reason instanceof Error ? friendlyAgentError(reason.message) : "درخواست دستیار ناموفق بود.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 sm:px-8" dir="rtl">
      <div className="mx-auto max-w-[1500px] space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start"><HermesSessionPanel projects={projects} selectedProjectId={selectedProjectId} onProjectChange={setSelectedProjectId} status={sessionStatus} proposal={proposal} />
