...
import { confirmScenarioCurrencyChange } from "../../lib/scenario-currency";
  ...
  return <form onSubmit={(event) => { event.preventDefault(); if (canSubmit) onAnalyze(); }} className="space-y-4">
    ...
    <Section title="دامنه‌ها و اتصال تیم / پروژه" description="گیت‌های کاتالوگ صرفاً هشدارند. انتخاب دامنهٔ gold دستی ممکن است؛ gold-launch به‌عنوان سناریوی آماده وجود ندارد.">
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <label className={labelClass}>ارز واحد سناریو<select value={value.currency} onChange={(event) => {
          const currency = event.target.value as ScenarioRequestDraft["currency"];
          const changed = confirmScenarioCurrencyChange(value, currency, (message) => window.confirm(message));
          if (changed) onChange(changed);
        }} className={inputClass}><option value="TOMAN">تومان</option><option value="USD">دلار آمریکا</option><option value="IQD">دینار عراق</option></select></label>
      ...
      <p role="note" className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs leading-6 text-amber-950">با تغییر ارز، مبالغ مالی و تبدیل‌های دستیِ هر سه حالت خالی می‌شوند و باید در ارز تازه دوباره وارد شوند. هیچ تبدیل خودکار یا نرخ پنهانی اعمال نمی‌شود.</p>
