...
export function CommandCenterShell() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 sm:px-8" dir="rtl"><div className="mx-auto max-w-[1500px] space-y-6"><header className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold text-cyan-600">مقر فرماندهی / نمای اجرایی</p><h1 className="mt-2 text-3xl font-black tracking-tight">اتاق فرمان پروژه‌ها</h1><p className="mt-2 text-sm text-slate-500">نمای یکپارچهٔ نیوکاش، شاطی و تراز با چهار فاز محصول: محصول، طراحی، توسعه و تحویل</p></div><div className="flex gap-2"><Badge tone="info">{snapshot.projects.length} پروژه</Badge><Badge tone="success">{snapshot.tasks.length} فعالیت</Badge></div></header>{error && <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">دریافت اطلاعات فرماندهی ناموفق بود؛ اتصال و نشست خود را بررسی کنید.</p>}<section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><FilterBar filters={filters} projects={snapshot.projects} teams={teams} users={users} onChange={onFilter} /></section><section><TimelineGrid tasks={tasks} range={range} zoom={(filters.zoom as "day" | "week" | "month" | undefined) ?? "day"} onSelectTask={setSelected} /></section><TaskDetailSheet task={selected} onClose={() => setSelected(null)} onRefresh={() => void load()} /></div></main>;
}
